...
export const generateProject = (
  ...
  return new Promise((resolve, reject) => {
    ...
    const dependencies: string[] = ["@namestys/templates"];
    ...
    executeCommand(command, args, root)
      .then(() => {
        copyAsync(
          `${root}/node_modules/@namestys/templates/${type}/${lang}`,
          `${root}`,
          {
            overwrite: true,
          }
          ...
          .then(() => {
            // Now need to remove `@namestys/templates` from packages
            if (useYarn) {
              command = "yarnpkg";
          ...
          .catch(() => {
            reject({
              command: `copy ${root}/node_modules/@namestys/templates/${type}/${lang} ${root}`,
            });
          });
      })
      .catch((error) => {
